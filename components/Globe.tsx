...
function RegionMarker({ region, selected, onClick }: { region: Region; selected: boolean; onClick: () => void }) {
  const ref = useRef<any>(null);
  const pos = latLon(region.lat, region.lon);
  useFrame((state: any) => {
    if (ref.current) {
      const s = selected ? 1.28 + Math.sin(state.clock.elapsedTime * 4) * .08 : 1;
      ref.current.scale.setScalar(s * region.scale);
    }
  });
  return <mesh ref={ref} position={[pos.x, pos.y, pos.z]} onClick={(e: any) => { e.stopPropagation(); onClick(); }}>
    <sphereGeometry args={[selected ? .105 : .055, 16, 16]} />
    <meshStandardMaterial color={selected ? '#0f9f6e' : '#6f8b83'} emissive={selected ? '#0f9f6e' : '#24443c'} emissiveIntensity={selected ? 1.5 : .3} />
  </mesh>;
}
...
function GlobeScene({ selected, onSelect }: { selected: string; onSelect: (name: string) => void }) {
  ...
  const down = (e: any) => { dragging.current = true; last.current = { x: e.clientX, y: e.clientY }; };
  const move = (e: any) => {
    if (!dragging.current || !group.current) return;
    group.current.rotation.y += (e.clientX - last.current.x) * .006;
    group.current.rotation.x += (e.clientY - last.current.y) * .003;
    group.current.rotation.x = Math.max(-.65, Math.min(.65, group.current.rotation.x));
  ...
  return <group ref={group} onPointerDown={down} onPointerUp={() => { dragging.current = false; }} onPointerMove={move}>
    <mesh>
      ...
      <meshStandardMaterial color="#eaf2ef" roughness={1} metalness={0} />
    </mesh>
    <mesh>
      <sphereGeometry args={[2.015, 48, 48]} />
      <meshBasicMaterial color="#6e8e83" wireframe transparent opacity={.17} />
    </mesh>
    <mesh>
      <sphereGeometry args={[2.03, 96, 96]} />
      <meshBasicMaterial color="#b9d0c8" transparent opacity={.06} wireframe />
    </mesh>
...
export default function Globe({ location, onLocation }: { location: string; onLocation: (name: string) => void }) {
  return <div className="globe">
    <Canvas camera={{ position: [0, .15, 6.1], fov: 35 }}>
      <ambientLight intensity={2.2} />
      <directionalLight position={[4, 3, 5]} intensity={2.5} />
      <GlobeScene selected={location} onSelect={onLocation} />
    </Canvas>
