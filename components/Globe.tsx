...
export default function Globe({location,onLocation,leads=[]}:Props){
 ...
 const geo=useMemo(()=>feature(countries as any,(countries as any).objects.countries).features as Country[],[]);
 ...
 useEffect(()=>{const f=focus[location];if(f){setRotation([f[0],f[1]]);setZoom(location?1.18:1)}} , [location]);
